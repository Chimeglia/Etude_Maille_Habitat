...
# Inclure le lien vers le CDN de Leaflet
leaflet_cdn <- tags$script(src = "https://unpkg.com/leaflet@1.9.3/dist/leaflet-src.js",
                           integrity = "sha512-XXX", crossorigin = "")
head_content <- list(leaflet_cdn)
...
```{r carteInter1,eval=T, echo=F,fig.align='center',fig.show='hold' ,fig.width = 9, fig.height = 9}
# Créer une nouvelle colonne contenant le texte à afficher dans les fenêtres contextuelles
...
carte <- leaflet(carteCommunesCorses_sf) %>%
#  addTiles() %>%
...
carte <- addLabelOnlyMarkers(carte,
                             data = centroids,
                             lng = ~coords[,1],
                             lat = ~coords[,2],
...
# Afficher la carte
knitr::include_graphics("aaa.jpg")
```
...
[Texte du lien ou bouton](carte_interactive1.html)
...
```{r trace_arbre_poids_minimal, eval=F, echo=F,fig.align='center',fig.show='hold' ,fig.width = 9, fig.height = 9}
...
for (i in seq_len(nrow(arbre_poids_minimal))) {
    carte <- addPolylines(carte, lng = c(coords[arbre_poids_minimal[i,1],1], coords[arbre_poids_minimal[i,2],1]), lat = c(coords[arbre_poids_minimal[i,1],2], coords[arbre_poids_minimal[i,2],2]), color = "red", weight = 1.5)
}
...
```{r trace_carteMaille1,eval=F, echo=F,fig.align='center',fig.show='hold' ,fig.width = 9, fig.height = 9}
...
```{r traceSecteurs,eval=F, echo=F,fig.align='center',fig.show='hold' ,fig.width = 9, fig.height = 9}
...
library(htmlwidgets)
load("Données/Donnees_Logement_2017.RData")
importsAirDNA <- read_excel("Données/stat_airdna_aout2020.xlsx", range = "A4:I364")
Import_carteCommunesCorses=st_read("Données/SHP_COMMUNES/COMMUNE.shp")
carteCommunesCorses=Import_carteCommunesCorses %>% select(INSEE_COM,geometry)
colnames(carteCommunesCorses)[1]="CODGEO"
carteCommunesCorses=carteCommunesCorses %>% arrange(CODGEO)
